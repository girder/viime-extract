...
def merge_lists(l1: list | None, l2: list | None) -> list:
    return (l1 or []) + (l2 or [])
...
class Person(BaseModel):
    ...
    firstname: str = Field(
        ..., title="First name", description="The first name of the person."
    )
    middle_initial: Optional[str] = Field(
        None,
        title="Middle initial",
        description="The middle initial of the person if present.",
    )
...
class ArticleKeyWords(BaseModel):
    """
    Represents the various keywords found in a journal article.
    """

    mentioned_metabolites: Optional[List[Metabolite]] = Field(
        title="Mentioned metabolites",
...
class ArticleMeta(BaseModel):
    """
    Represents the metadata of a journal article.
    """
    ...
    journal: str = Field(
        ...,
        title="Journal",
        description="The journal in which the article was published.",
    )
    year: int = Field(
        ..., title="Year", description="The year in which the article was published."
    )
    volume: str = Field(
        ...,
        title="Volume",
        description="The volume of the journal in which the article was published.",
    )
    pubmed_id: Optional[str] = Field(
        ..., title="PubMed ID", description="The PubMed ID of the article"
    )
    doi_id: Optional[str] = Field(
        ..., title="DOI ID", description="The DOI ID of the article"
    )
    authors: List[Person] = Field(
        ..., title="Authors", description="The authors of the article."
    )
