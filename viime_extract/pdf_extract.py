...
to include an entity with an "unknown" identified than to omit it entirely.""",
    ...
    for text_chunk in splitter.split_documents(pages):
        prompt = template.invoke({"article_contents": text_chunk.page_content})
...
def extract_article_from_document_loader(
    *,
    ...
    if without_references:
        limit = get_start_of_references_idx(pages, model)
        logging.info("Ignoring pages starting with index %d", limit)
    else:
        limit = len(pages)
    meta = extract_article_metadata(pages[:limit], model)
    keywords = extract_article_keywords(pages[:limit], model, splitter)
